...
| `weo_YYYY_MM` | IMF WEO database vintages (Apr+Oct editions 2018→): each edition's bulk file trimmed at retrieval to the `NGDP_RPCH` rows (real GDP growth, all countries, UTF-8); years beyond the edition's own `Estimates Start After` become `imf_weo` forecast rows (US mapped; other regions await X4). `forecast_date` approximated as the 15th of the edition month | **calendar-year avg %** (stored `yoy_pct`, like `spf_ecb` annual rows); `value_qq` null by design | Blob layout/case changed thrice (year-root → `/{01,02}/` → `/{month}/`, now all-lowercase); legacy files are cp1252, modern UTF-16LE — the trim hook validates payloads so CDN 200-with-error-page responses fall through to the next candidate URL | edition download pages list the live link; SDMX WEO API (current edition only) |
...
| IMF (WEO database) | [IMF copyright & usage](https://www.imf.org/en/About/copyright-and-terms) | free for non-commercial use with attribution; **verify before bulk redistribution** (cache holds only the trimmed NGDP_RPCH slice per edition) |
